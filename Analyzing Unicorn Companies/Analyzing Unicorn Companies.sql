...
WITH top_industries AS(
    SELECT 
        industry, EXTRACT(YEAR FROM date_joined) as year, COUNT(industry) as num_unicorns,
        ROUND(AVG(valuation)/1000000000, 2)as average_valuation_billions
    FROM industries i, dates d, funding f
    WHERE i.company_id = d.company_id
    AND i.company_id = f.company_id
    AND EXTRACT(YEAR FROM date_joined) BETWEEN 2019 AND 2021
    GROUP BY industry, EXTRACT(YEAR FROM date_joined) ), 
    
ranking_cte AS(
    SELECT *, DENSE_RANK() OVER(ORDER BY sum_unicorns desc) as ranking
    FROM (
